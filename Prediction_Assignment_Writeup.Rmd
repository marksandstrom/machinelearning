...
Also, the model `m` trained by the above script predicted correctly each of the 20 'blind' validation test cases in https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv, as follows:

```{r}
pv
```
The model `m` thus meets its objectives for this assignment, and there is no need to explore alternative or more complex models. 

Random forest models are often, as in the present case, good for a balance between good accuracy and computational efficiency. Given that the model training involves a number of trees, they are not best for interpretability or visualization. However, we can view the the most important predictor variables used in training m:
```{r}
varImpPlot(m, n.var=min(20, nrow(m$importance)))
```

Top 20 predictors for the random forest model m by importance; for explanation of the Gini importance ranking, please see: https://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#giniimp
